...
> Được giới thiệu trong [Linux and PEAK-System's CAN Interfaces](https://www.peak-system.com/fileadmin/media/linux/index.php), đây là phiên bản dịch lại từ tệp tài liệu [SocketCAN Documentation](https://www.kernel.org/doc/Documentation/networking/can.txt) cái mà mình cũng không rõ nó từ đâu nhưng có vẻ nó được phát hành từ trang [kernel.org](https://docs.kernel.org/) của **Linux**, khá uy tín. Đây chỉ là tài liệu hệ thống nên hơi khô khan, mục đích chính của nhiệm vụ này là dịch lại, đọc và tìm hiểu về hệ thống này.

_Tài liệu `can.txt`_

Tập tin readme dành cho **Controller Area Network Protocol Family (aka SocketCAN)**
...
=== "VI"
    Gói socketcan là triển khai các giao thức CAN **(Controller Area Network)** cho Linux. CAN là một công nghệ mạng được sử dụng rộng rãi trong lĩnh vực tự động hóa, thiết bị nhúng và ô tô. Mặc dù đã có các triển khai CAN khác cho Linux dựa trên các thiết bị ký tự, nhưng SocketCAN sử dụng API ổ cắm Berkeley, ngăn xếp mạng Linux và triển khai trình điều khiển thiết bị CAN làm giao diện mạng. API CAN socket đã được thiết kế giống nhất có thể với các giao thức TCP/IP để cho phép các lập trình viên, quen thuộc với lập trình mạng, dễ dàng tìm hiểu cách sử dụng socket CAN.
=== "EN"
...
=== "VI"
    Đã có triển khai CAN cho Linux trước SocketCAN nên câu hỏi đặt ra là tại sao chúng tôi lại bắt đầu một dự án khác. Hầu hết các triển khai hiện có đều có vai trò là trình điều khiển thiết bị cho một số phần cứng CAN, chúng dựa trên các thiết bị ký tự và cung cấp tương đối ít chức năng. Thông thường, chỉ có trình điều khiển thiết bị dành riêng cho phần cứng cung cấp giao diện thiết bị ký tự để gửi và nhận các khung CAN thô, trực tiếp đến/từ phần cứng bộ điều khiển. Việc xếp hàng các khung và các giao thức truyền tải cấp cao hơn như ISO-TP phải được triển khai trong các ứng dụng không gian người dùng. Ngoài ra, hầu hết việc triển khai thiết bị ký tự chỉ hỗ trợ một quy trình duy nhất để mở thiết bị tại một thời điểm, tương tự như giao diện nối tiếp. Việc trao đổi bộ điều khiển CAN yêu cầu sử dụng trình điều khiển thiết bị khác và thường cần phải điều chỉnh phần lớn ứng dụng sang API của trình điều khiển mới.

    SocketCAN được thiết kế để khắc phục tất cả những hạn chế này. Một họ giao thức mới đã được triển khai nhằm cung cấp giao diện ổ cắm _(socket interface)_ cho các ứng dụng trong không gian người dùng và được xây dựng trên lớp mạng Linux, cho phép sử dụng tất cả chức năng xếp hàng được cung cấp. Trình điều khiển thiết bị cho phần cứng bộ điều khiển CAN tự đăng ký với lớp mạng Linux dưới dạng thiết bị mạng, do đó các khung CAN từ bộ điều khiển có thể được chuyển lên lớp mạng và sang mô-đun dòng giao thức CAN và ngược lại. Ngoài ra, mô-đun họ giao thức còn cung cấp một API để các mô-đun giao thức truyền tải đăng ký, sao cho bất kỳ số lượng giao thức truyền tải nào cũng có thể được tải hoặc dỡ tải một cách linh hoạt. Trên thực tế, chỉ riêng mô-đun lõi can không cung cấp bất kỳ giao thức nào và không thể sử dụng được nếu không tải ít nhất một mô-đun giao thức bổ sung. Nhiều ổ cắm có thể được mở cùng lúc, trên cùng một mô-đun giao thức hoặc khác nhau và chúng có thể nghe/gửi các khung trên các ID CAN khác nhau hoặc giống nhau. Một số ổ cắm nghe trên cùng một giao diện cho các khung có cùng ID CAN đều được truyền cùng các khung CAN phù hợp nhận được. Một ứng dụng muốn giao tiếp bằng một giao thức truyền tải cụ thể, ví dụ: ISO-TP, chỉ cần chọn giao thức đó khi mở ổ cắm, sau đó có thể đọc và ghi các luồng byte dữ liệu ứng dụng mà không cần phải xử lý CAN-ID, khung, v.v.

    Chức năng tương tự hiển thị từ không gian người dùng cũng có thể được cung cấp bởi thiết bị ký tự, nhưng điều này sẽ dẫn đến một giải pháp không phù hợp về mặt kỹ thuật vì một số lý do:

    - **Cách sử dụng phức tạp**. Thay vì chuyển đối số giao thức tới `socket(2)` và sử dụng `bind(2)` để chọn giao diện **CAN** và **CAN ID**, ứng dụng sẽ phải thực hiện tất cả các thao tác này bằng `ioctl(2)s`.
    - **Sao chép mã**. Một thiết bị ký tự không thể sử dụng mã xếp hàng mạng Linux, vì vậy tất cả mã đó sẽ phải được sao chép cho mạng CAN.
    - **Trừu tượng**. Trong hầu hết các triển khai thiết bị ký tự hiện có, trình điều khiển thiết bị dành riêng cho phần cứng cho bộ điều khiển CAN trực tiếp cung cấp thiết bị ký tự để ứng dụng hoạt động. Điều này ít nhất là rất bất thường trong các hệ thống Unix cho cả thiết bị char và block. Ví dụ: bạn không có thiết bị ký tự cho một UART nhất định của giao diện nối tiếp, chip âm thanh nhất định trong máy tính, bộ điều khiển SCSI hoặc IDE cung cấp quyền truy cập vào đĩa cứng hoặc thiết bị truyền phát băng của bạn. Thay vào đó, bạn có các lớp trừu tượng một mặt cung cấp ký tự thống nhất hoặc giao diện thiết bị khối cho ứng dụng và mặt khác là giao diện cho trình điều khiển thiết bị dành riêng cho phần cứng. Những phần trừu tượng này được cung cấp bởi các hệ thống con như lớp tty, hệ thống con âm thanh hoặc hệ thống con SCSI và IDE cho các thiết bị được đề cập ở trên.

    Cách dễ nhất để triển khai trình điều khiển thiết bị CAN là dưới dạng một thiết bị ký tự không có lớp trừu tượng (hoàn chỉnh) như hầu hết các trình điều khiển hiện có. Tuy nhiên, cách đúng đắn là thêm một lớp như vậy với tất cả các chức năng như đăng ký một số ID CAN nhất định, hỗ trợ một số bộ mô tả tệp mở và (de) ghép kênh các khung CAN giữa chúng, xếp hàng (tinh vi) các khung CAN và cung cấp API để trình điều khiển thiết bị đăng ký. Tuy nhiên, khi đó sẽ không còn khó khăn nữa hoặc thậm chí có thể dễ dàng hơn khi sử dụng khung mạng do nhân Linux cung cấp và đây chính là điều mà SocketCAN thực hiện.

    Việc sử dụng framework mạng của nhân Linux chỉ là cách tự nhiên và thích hợp nhất để triển khai CAN cho Linux.
=== "EN"
...
=== "VI"
    Như đã mô tả trong chương 2, mục tiêu chính của SocketCAN là cung cấp giao diện socket cho các ứng dụng không gian người dùng được xây dựng trên lớp mạng Linux. Ngược lại với mạng TCP/IP và ethernet thường được biết đến, <mark>bus CAN là phương tiện chỉ phát sóng</mark> Không có địa chỉ lớp MAC như ethernet. Mã định danh CAN (can_id) được sử dụng để phân xử trên CAN-bus. Do đó CAN-ID phải được chọn duy nhất trên _bus_. Khi thiết kế mạng CAN-ECU, CAN-ID được ánh xạ để gửi bởi một ECU cụ thể. Vì lý do này, CAN-ID có thể được coi là một loại địa chỉ nguồn tốt nhất.
=== "EN"
    As described in chapter 2 it is the main goal of SocketCAN to provide a socket interface to user space applications which builds upon the Linux network layer. In contrast to the commonly known TCP/IP and ethernet networking, <mark>the CAN bus is a broadcast-only</mark> medium that has no MAC-layer addressing like ethernet. The CAN-identifier (can_id) is used for arbitration on the CAN-bus. Therefore the CAN-IDs have to be chosen uniquely on the bus. When designing a CAN-ECU network the CAN-IDs are mapped to be sent by a specific ECU. For this reason a CAN-ID can be treated best as a kind of source address.

### 3.1 receive lists

=== "VI"
    Việc truy cập trong suốt mạng của nhiều ứng dụng dẫn đến vấn đề là các ứng dụng khác nhau có thể quan tâm đến cùng một CAN-ID từ cùng một giao diện mạng CAN. Mô-đun lõi của SocketCAN - triển khai họ giao thức CAN - cung cấp một số danh sách nhận hiệu quả cao vì lý do này. Nếu ví dụ: một ứng dụng không gian người dùng sẽ mở một ổ cắm CAN RAW, mô-đun giao thức thô sẽ tự yêu cầu (phạm vi) CAN-ID từ lõi SocketCAN mà người dùng yêu cầu. Việc đăng ký và hủy đăng ký CAN-ID có thể được thực hiện cho các giao diện CAN cụ thể hoặc cho tất cả (!) giao diện CAN đã biết với các hàm can_rx_(un)register() được cung cấp cho các mô-đun giao thức CAN bởi lõi SocketCAN (xem chương 5). Để tối ưu hóa việc sử dụng CPU trong thời gian chạy, danh sách nhận được chia thành nhiều danh sách cụ thể cho mỗi thiết bị phù hợp với độ phức tạp của bộ lọc được yêu cầu cho một trường hợp sử dụng nhất định.
=== "EN"
...
=== "VI"
    Như đã biết từ các khái niệm mạng khác, các ứng dụng trao đổi dữ liệu có thể chạy trên cùng một nút hoặc các nút khác nhau mà không có bất kỳ thay đổi nào (ngoại trừ thông tin địa chỉ theo):
    ...
    Để đảm bảo rằng ứng dụng A nhận được thông tin tương tự trong ví dụ (2) như nó sẽ nhận được trong ví dụ (1), cần có một số loại vòng lặp cục bộ của các khung CAN đã gửi trên nút thích hợp.

    Các thiết bị mạng Linux (theo mặc định) chỉ có thể xử lý việc truyền và nhận các khung phụ thuộc vào phương tiện. Do sự phân xử trên bus CAN nên việc truyền CAN-ID có mức ưu tiên thấp có thể bị trì hoãn do việc nhận khung CAN có mức ưu tiên cao. Để phản ánh lưu lượng truy cập* chính xác trên nút, việc lặp lại dữ liệu đã gửi phải được thực hiện ngay sau khi truyền thành công. Nếu giao diện mạng CAN không có khả năng thực hiện vòng lặp vì lý do nào đó thì lõi SocketCAN có thể thực hiện nhiệm vụ này như một giải pháp dự phòng. Xem chương 6.2 để biết chi tiết (được khuyến nghị).

    Chức năng loopback được bật theo mặc định để phản ánh hành vi kết nối mạng tiêu chuẩn cho các ứng dụng CAN. Do một số yêu cầu từ nhóm RT-SocketCAN, tùy chọn vòng lặp có thể bị tắt đối với từng ổ cắm riêng biệt. Xem các lỗ cắm từ ổ cắm CAN RAW ở chương 4.1.

    !!! note "Note"
        bạn thực sự muốn có điều này khi bạn đang chạy các công cụ phân tích như 'candump' hoặc 'cansniffer' trên cùng một nút.
...
=== "VI"
    Việc sử dụng bus CAN có thể dẫn đến một số vấn đề trên lớp điều khiển truy cập vật lý và phương tiện. Việc phát hiện và ghi nhật ký các sự cố lớp thấp hơn này là yêu cầu quan trọng đối với người dùng CAN để xác định các sự cố phần cứng trên lớp thu phát vật lý cũng như các sự cố trọng tài và khung lỗi do các ECU khác nhau gây ra. Sự xuất hiện của các lỗi được phát hiện rất quan trọng đối với việc chẩn đoán và phải được ghi lại cùng với dấu thời gian chính xác. Vì lý do này, trình điều khiển giao diện CAN có thể tạo ra cái gọi là Khung thông báo lỗi có thể được chuyển đến ứng dụng người dùng theo cách tương tự như các khung CAN khác. Bất cứ khi nào phát hiện thấy lỗi trên lớp vật lý hoặc lớp MAC (ví dụ: bởi bộ điều khiển CAN), trình điều khiển sẽ tạo một khung thông báo lỗi thích hợp. Ứng dụng người dùng có thể yêu cầu các khung thông báo lỗi bằng cách sử dụng các cơ chế lọc CAN phổ biến. Bên trong định nghĩa bộ lọc này, loại lỗi (quan tâm) có thể được chọn. Việc nhận thông báo lỗi bị tắt theo mặc định. Định dạng của khung thông báo lỗi CAN được mô tả ngắn gọn trong tệp tiêu đề Linux `"include/uapi/linux/can/error.h"`.
=== "EN"
...
=== "VI"
    Giống như TCP/IP, trước tiên bạn cần mở một ổ cắm để liên lạc qua mạng CAN. Vì SocketCAN triển khai một họ giao thức mới nên bạn cần chuyển PF_CAN làm đối số đầu tiên cho lệnh gọi hệ thống socket(2). Hiện tại, có hai giao thức CAN để lựa chọn, giao thức ổ cắm thô và trình quản lý _broadcast_ (BCM). Vì vậy, để mở một socket, bạn sẽ viết:
    ...
    tương ứng. Sau khi tạo ổ cắm thành công, thông thường bạn sẽ sử dụng lệnh gọi hệ thống bind(2) để liên kết ổ cắm với giao diện CAN (khác với giao diện TCP/IP do địa chỉ khác nhau - xem chương 3). Sau khi liên kết (CAN_RAW) hoặc kết nối (CAN_BCM) ổ cắm, bạn có thể đọc (2) và ghi (2) từ/vào ổ cắm hoặc sử dụng các thao tác send(2), sendto(2), sendmsg(2) và recv* trên ổ cắm như bình thường. Ngoài ra còn có các tùy chọn ổ cắm cụ thể CAN được mô tả bên dưới.
    
    Cấu trúc khung CAN cơ bản và cấu trúc sockaddr được xác định trong `include/linux/can.h`:
    ...
    Việc căn chỉnh dữ liệu tải trọng (tuyến tính)[] theo ranh giới 64 bit cho phép người dùng xác định cấu trúc và liên kết của riêng họ để dễ dàng truy cập tải trọng CAN. Theo mặc định, không có thứ tự byte nhất định trên bus CAN. Lệnh gọi hệ thống đọc (2) trên ổ cắm CAN_RAW sẽ chuyển cấu trúc can_frame sang không gian người dùng.
    
    Cấu trúc **sockaddr_can** có chỉ mục giao diện giống như ổ cắm PF_PACKET, cũng liên kết với một giao diện cụ thể:
    ...
    Để xác định chỉ mục giao diện, phải sử dụng `ioctl()` thích hợp (ví dụ đối với ổ cắm **CAN_RAW** không kiểm tra lỗi):
    ...
    <mark>Để liên kết một ổ cắm với tất cả các giao diện CAN</mark> Chỉ số giao diện phải bằng 0 (không). Trong trường hợp này, ổ cắm nhận các khung CAN từ mọi giao diện CAN được bật. Để xác định giao diện CAN gốc, lệnh gọi hệ thống recvfrom(2) có thể được sử dụng thay vì read(2). Để gửi trên một ổ cắm được liên kết với giao diện 'bất kỳ', sendto(2) là cần thiết để chỉ định giao diện gửi đi.
    
    Việc đọc các khung CAN từ một socket CAN_RAW bị ràng buộc (xem ở trên) bao gồm việc đọc một cấu trúc can_frame:
    ...
    Khi giao diện CAN được liên kết với 'bất kỳ' CAN interface hiện có `(addr.can_ifindex = 0)` nên sử dụng `recvfrom(2)` nếu cần thông tin về giao diện CAN gốc:
    ...
    Để ghi các khung CAN trên các ổ cắm được liên kết với giao diện CAN 'bất kỳ', giao diện gửi đi phải được xác định chắc chắn.
    ...
    Có thể lấy dấu thời gian chính xác bằng lệnh gọi ioctl(2) sau khi đọc tin nhắn từ ổ cắm:
    ...
    Dấu thời gian có độ phân giải một micro giây và được đặt tự động khi nhận khung CAN.
    
    Nhận xét về hỗ trợ CAN FD (tốc độ dữ liệu linh hoạt):
    
    Nói chung việc xử lý CAN FD rất giống với các ví dụ được mô tả trước đây. Bộ điều khiển CAN có khả năng CAN FD mới hỗ trợ hai tốc độ bit khác nhau cho giai đoạn phân xử và giai đoạn tải trọng của khung CAN FD và tải trọng lên tới 64 byte. Độ dài tải trọng mở rộng này phá vỡ tất cả các giao diện hạt nhân (ABI) phụ thuộc nhiều vào khung CAN với tám byte tải trọng cố định (struct can_frame) như ổ cắm CAN_RAW. Vì vậy, ví dụ: ổ cắm CAN_RAW hỗ trợ tùy chọn ổ cắm mới **CAN_RAW_FD_FRAMES** để chuyển ổ cắm sang chế độ cho phép xử lý đồng thời các khung CAN FD và khung CAN (cũ) (xem phần 4.1.5).
    ...
    Cấu trúc canfd_frame và cấu trúc can_frame hiện có có can_id, độ dài tải trọng và dữ liệu tải trọng ở cùng một độ lệch bên trong cấu trúc của chúng. Điều này cho phép xử lý các cấu trúc khác nhau rất giống nhau. Khi nội dung của struct can_frame được sao chép vào struct canfd_frame, tất cả các thành phần cấu trúc có thể được sử dụng nguyên trạng - chỉ có data[] mới được mở rộng.
    
    Khi giới thiệu struct canfd_frame, hóa ra mã độ dài dữ liệu (DLC) của struct can_frame được sử dụng làm thông tin độ dài vì độ dài và DLC có ánh xạ 1: 1 trong phạm vi 0 .. 8. Để duy trì việc xử lý dễ dàng thông tin độ dài, phần tử canfd_frame.len chứa giá trị độ dài đơn giản từ 0 .. 64. Vì vậy, cả canfd_frame.len và can_frame.can_dlc đều bằng nhau và chứa độ dài thông tin và không có DLC. Để biết chi tiết về sự khác biệt giữa các thiết bị có khả năng CAN và CAN FD cũng như ánh xạ tới mã chiều dài dữ liệu liên quan đến bus (DLC), hãy xem chương 6.6.
    
    Độ dài của hai cấu trúc khung CAN(FD) xác định đơn vị truyền tối đa (MTU) của giao diện mạng CAN(FD) và độ dài dữ liệu skbuff. Hai định nghĩa được chỉ định cho các MTU cụ thể của CAN trong `include/linux/can.h`:
...
=== "EN"
    ...
    respectively. After the successful creation of the socket, you would normally use the bind(2) system call to bind the socket to a CAN interface (which is different from TCP/IP due to different addressing - see chapter 3). After binding (CAN_RAW) or connecting (CAN_BCM) the socket, you can read(2) and write(2) from/to the socket or use send(2), sendto(2), sendmsg(2) and the recv* counterpart operations on the socket as usual. There are also CAN specific socket options described below.
...
=== "VI"
    Việc sử dụng ổ cắm CAN_RAW có thể so sánh rộng rãi với việc truy cập phổ biến vào các thiết bị ký tự CAN. Để đáp ứng các khả năng mới được cung cấp bởi phương pháp SocketCAN nhiều người dùng, một số giá trị mặc định hợp lý được đặt ở thời gian liên kết ổ cắm RAW:
    ...
    - Tính năng lặp lại các khung CAN đã gửi được bật (xem chương 3.2)
    - Ổ cắm không nhận được các khung đã gửi của chính nó (ở chế độ loopback)

    Các cài đặt mặc định này có thể được thay đổi trước hoặc sau khi liên kết ổ cắm. Để sử dụng các định nghĩa được tham chiếu của các tùy chọn ổ cắm cho ổ cắm CAN_RAW, hãy `include <linux/can/raw.h>`.
=== "EN"
...
=== "VI"
    Việc tiếp nhận khung CAN sử dụng ổ cắm CAN_RAW có thể được kiểm soát bằng cách xác định 0 .. n bộ lọc với tùy chọn ổ cắm CAN_RAW_FILTER.
    ...
    Một bộ lọc phù hợp, khi `<received_can_id> & mask == can_id & mask`

    tương tự với ngữ nghĩa của bộ lọc phần cứng bộ điều khiển CAN đã biết. Bộ lọc có thể được đảo ngược theo ngữ nghĩa này, khi bit CAN_INV_FILTER được đặt trong phần tử can_id của cấu trúc can_filter. Ngược lại với các bộ lọc phần cứng của bộ điều khiển CAN, người dùng có thể đặt 0 .. n nhận các bộ lọc cho từng ổ cắm mở riêng biệt:
    ...
    Để đặt bộ lọc về 0, bộ lọc khá lỗi thời vì việc không đọc dữ liệu khiến ổ cắm thô loại bỏ các khung CAN nhận được. Nhưng với trường hợp sử dụng 'chỉ gửi' này, chúng tôi có thể xóa danh sách nhận trong Kernel để tiết kiệm một ít (thực sự là rất ít!) mức sử dụng CPU.
=== "EN"
...
=== "VI"
    Bộ lọc CAN được xử lý trong danh sách bộ lọc trên mỗi thiết bị tại thời điểm nhận khung CAN. Để giảm số lượng kiểm tra cần thực hiện trong khi xem qua danh sách bộ lọc, lõi CAN cung cấp khả năng xử lý bộ lọc được tối ưu hóa khi đăng ký bộ lọc tập trung vào một ID CAN duy nhất.

    Đối với các số nhận dạng 2048 SFF CAN có thể, số nhận dạng này được sử dụng làm chỉ mục để truy cập vào danh sách đăng ký tương ứng mà không cần kiểm tra thêm. Đối với 2^29 mã định danh EFF CAN có thể có, phép gấp XOR 10 bit được sử dụng làm hàm băm để truy xuất chỉ mục bảng EFF.

    Để hưởng lợi từ các bộ lọc được tối ưu hóa cho các mã định danh CAN đơn, CAN_SFF_MASK hoặc CAN_EFF_MASK phải được đặt thành can_filter.mask cùng với các bit CAN_EFF_FLAG và CAN_RTR_FLAG được đặt. Một bit CAN_EFF_FLAG được đặt trong can_filter.mask cho biết rõ rằng việc đăng ký SFF hay EFF CAN ID có quan trọng hay không. Ví dụ. trong ví dụ ở trên
    ...
    cả hai khung SFF có CAN ID 0x123 và khung EFF có 0xXXXXX123 đều có thể vượt qua.

    Để chỉ lọc các mã định danh CAN 0x123 (SFF) và 0x12345678 (EFF), bộ lọc phải được xác định theo cách này để hưởng lợi từ các bộ lọc được tối ưu hóa:
...
=== "VI"
    Như được mô tả trong chương 3.3, trình điều khiển giao diện CAN có thể tạo ra cái gọi là Khung thông báo lỗi có thể được chuyển đến ứng dụng người dùng theo cách tùy chọn giống như các khung CAN khác. Các lỗi có thể xảy ra được chia thành các loại lỗi khác nhau có thể được lọc bằng mặt nạ lỗi thích hợp. Để đăng ký mọi điều kiện lỗi có thể xảy ra, CAN_ERR_MASK có thể được sử dụng làm giá trị cho mặt nạ lỗi. Các giá trị cho mặt nạ lỗi được xác định trong `linux/can/error.h`.
...
=== "VI"
    Để đáp ứng nhu cầu của nhiều người dùng, vòng lặp cục bộ được bật theo mặc định (xem chương 3.2 để biết chi tiết). Nhưng trong một số trường hợp sử dụng được nhúng (ví dụ: khi chỉ có một ứng dụng sử dụng bus CAN), chức năng lặp lại này có thể bị tắt (riêng cho từng ổ cắm):
...
=== "VI"
    Khi tính năng vòng lặp cục bộ được bật, tất cả các khung CAN đã gửi sẽ được lặp lại vào các ổ cắm CAN mở đã đăng ký CAN-ID của khung CAN trên giao diện nhất định này để đáp ứng nhu cầu của nhiều người dùng. Việc tiếp nhận các khung CAN trên cùng một ổ cắm đang gửi khung CAN được coi là không mong muốn và do đó bị tắt theo mặc định. Hành vi mặc định này có thể được thay đổi theo yêu cầu:
...
=== "VI"
    Hỗ trợ CAN FD trong ổ cắm CAN_RAW có thể được bật bằng tùy chọn ổ cắm mới **CAN_RAW_FD_FRAMES** tùy chọn này bị tắt theo mặc định. Khi tùy chọn ổ cắm mới không được ổ cắm CAN_RAW hỗ trợ (ví dụ: trên các hạt nhân cũ hơn), việc chuyển đổi tùy chọn **CAN_RAW_FD_FRAMES** sẽ trả về lỗi -ENOPROTOOPT.

    Khi **CAN_RAW_FD_FRAMES** được bật, ứng dụng có thể gửi cả khung CAN và khung CAN FD. OTOH ứng dụng phải xử lý các khung CAN và CAN FD khi đọc từ ổ cắm.

    - **CAN_RAW_FD_FRAMES** enabled:  CAN_MTU và CANFD_MTU được phép
    - **CAN_RAW_FD_FRAMES** disabled: chỉ CAN_MTU được phép (mặc định)

    Ví dụ: _**[remember: CANFD_MTU == sizeof(struct canfd_frame)]**_
    ...
    Khi đọc với kích thước CANFD_MTU chỉ trả về các byte CAN_MTU đã được nhận từ ổ cắm, khung CAN cũ đã được đọc vào cấu trúc CAN FD được cung cấp. Lưu ý rằng trường dữ liệu canfd_frame.flags không được chỉ định trong cấu trúc can_frame và do đó nó chỉ hợp lệ trong các khung CAN FD có kích thước CANFD_MTU.

    Gợi ý triển khai cho các ứng dụng CAN mới:

    Để xây dựng ứng dụng nhận biết CAN FD, hãy sử dụng struct canfd_frame làm cấu trúc dữ liệu CAN cơ bản cho các ứng dụng dựa trên CAN_RAW. Khi ứng dụng được thực thi trên nhân Linux cũ hơn và việc chuyển đổi tùy chọn ổ cắm **CAN_RAW_FD_FRAMES** sẽ trả về lỗi: Không vấn đề gì. Bạn sẽ nhận được các khung CAN hoặc khung CAN FD cũ và có thể xử lý chúng theo cách tương tự.

    Khi gửi tới các thiết bị CAN, hãy đảm bảo rằng thiết bị có khả năng xử lý các khung CAN FD bằng cách kiểm tra xem đơn vị truyền tối đa của thiết bị có phải là CANFD_MTU hay không. MTU của thiết bị CAN có thể được truy xuất, ví dụ: với một tòa nhà cao tầng SIOCGIFMTU ioctl().
=== "EN"
    ...
    Example: _**[remember: CANFD_MTU == sizeof(struct canfd_frame)]**_
...
- **MSG_CONFIRM**: set when the frame was sent via the socket it is received on. This flag can be interpreted as a 'transmission confirmation' when the CAN driver supports the echo of frames on driver level, see 3.2 and 6.2. In order to receive such messages, CAN_RAW_RECV_OWN_MSGS must be set.
...
| **RX_ANNOUNCE_RESUME** | passed at RX_SETUP and a receive timeout occurred, a RX_CHANGED message will be generated when the (cyclic) receive restarts.                                                                                                                                               |
| **TX_RESET_MULTI_IDX** | the index for the multiple frame transmission.                                                                                                                                                                                                                              |
...
### 4.3 connected transport protocols (SOCK_SEQPACKET)c
### 4.4 unconnected transport protocols (SOCK_DGRAM)
